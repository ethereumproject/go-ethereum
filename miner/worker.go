...
func (env *Work) commitTransactions(mux *event.TypeMux, transactions types.Transactions, gasPrice *big.Int, bc *core.BlockChain) {
	...
	if len(coalescedLogs) > 0 || env.tcount > 0 {
		go func(logs state.Logs, tcount int) {
			if len(logs) > 0 {
...
func (env *Work) commitTransaction(tx *types.Transaction, bc *core.BlockChain, gp *core.GasPool) (error, state.Logs) {
