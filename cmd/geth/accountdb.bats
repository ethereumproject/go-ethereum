...
@test "account import (db)" {
	run $GETH_CMD --datadir $DATA_DIR --lightkdf --index-accounts wallet import $BATS_TEST_DIRNAME/testdata/guswallet.json <<< $'foo\n'
	echo "$output"
...
@test "account import pass mismatch (db)" {
	run $GETH_CMD --datadir $DATA_DIR --lightkdf --index-accounts wallet import $BATS_TEST_DIRNAME/testdata/guswallet.json <<< $'wrong\n'
	echo "$output"
...
@test "account unlock ambiguous pass mismatch (db)" {
	...
	run $GETH_CMD --datadir $DATA_DIR --keystore $DATA_DIR/store --index-accounts --nat none --nodiscover --dev --unlock f466859ead1932d743d622cb74fc058882e8648a js $DATA_DIR/empty.js <<< $'wrong\n'$DATA_DIR/store/1
	echo "$output"
